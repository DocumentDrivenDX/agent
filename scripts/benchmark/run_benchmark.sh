...
PROVIDER_THINKING_LEVEL="${DDX_BENCH_PROVIDER_THINKING_LEVEL:-}"
PROVIDER_THINKING_BUDGET="${DDX_BENCH_PROVIDER_THINKING_BUDGET:-}"
...
if [[ -z "${DDX_AGENT_BINARY:-}" ]]; then
    ...
    GOOS=linux GOARCH=amd64 go build \
        ...
        -o "${DEFAULT_BINARY}" "${REPO_ROOT}/cmd/ddx-agent"
...
DRY RUN
  ...
  provider_thinking_budget=${PROVIDER_THINKING_BUDGET}
...
while IFS= read -r TASK_ID; do
    ...
    HARBOR_OUT="$( \
        ...
        DDX_BENCH_PROVIDER_THINKING_LEVEL="${PROVIDER_THINKING_LEVEL}" \
        DDX_BENCH_PROVIDER_THINKING_BUDGET="${PROVIDER_THINKING_BUDGET}" \
...
go run ./cmd/ddx-agent-benchscore -tasks-jsonl "${TASK_RESULTS_FILE}" > "${SCORES_FILE}"
